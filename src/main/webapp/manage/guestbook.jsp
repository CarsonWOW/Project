...
<head>
	...
	<script >
		function deleteFoods(id) {
			if (confirm("确定要删除吗?")){
				location.href = "deleteShops.shop?id=" + id;
			}
		}
	</script>
</head>
			...
			<c:forEach var="li" items="${list}">
				<tr>
					...
					<c:forEach var="u" items="${shopsTypes}">
						<c:if test="${li.shopTypeId==u.BU_shopTypeId}">
							<td>${u.BU_Name}</td>
						</c:if>
					</c:forEach>
		...
		<tr>
			<td>
				...
				<c:if test="${page.curPageNo>1}">
					<a href="${pageContext.request.contextPath}/Shop.shop?curPageNo=1"> 首页</a>
					<a  href="${pageContext.request.contextPath}/Shop.shop?curPageNo=${page.curPageNo-1}">上一页</a>
				</c:if>

				<c:if test="${page.curPageNo<page.totalPageCount}">
					<a  href="${pageContext.request.contextPath}/Shop.shop?curPageNo=${page.curPageNo+1}">下一页</a>
					<a href="${pageContext.request.contextPath}/Shop.shop?curPageNo=${page.totalPageCount}">末页</a>
				</c:if>
